...
enum Tool: String, Identifiable, CaseIterable, Codable {
    ...
    var toolPrompt: String {
        switch self {
        case .webSearch: 
            "If the user requests for information beyond your knowledge cutoff, current events, about some term you are totally unfamiliar with (it might be new), you may use the web search tool to find the information on the internet. BUt do not use this too to scrape links. Use the scrapeLinks tool for that. In many cases, a simple call of teh web search tool may be sufficient to find the information using the tools' short snippets. However, if the user requests for more detailed information or if the snippets and website titles were not sufficient to properly answer the user's query, you may use the scrapeLinks tool to look into some of the urls that were in the search results. If you used context from a certain snippet of a certain website, format the reference link properly using markdown at the end of your response. For referemnces, use a short display text instead of the website's entire title. Regardless, always prioritise your existing knwoledge from your training. Do not use the tool to look up information that you already know unless the user specifically asks for it. You usually wont need to use this tool twice at a time but perhaps if teh initial resulst are not satisfactory and you feel scraping links wont help either, you may call it again. If you are not sure if you need to search the web, feel free to ask the user for permission."
        case .scrapeLinks:
            "If links are provided by the user, you may use the scrapeLinks tool to fetch the contents of the URLs. The tool will fetch the contents of the URLs and return the text. Do not come up with Urls on your own. Only use the scrapeLinks tool if the user provides the links or if there are urls available in the chat from a prior web search tool invocation. The tool may often fail to retrieve the contents of the URL, in which case you should inform the user that you were unable to retrieve the contents of the URL. Moreover, content scraped from links may contain a lot of extra stuff such as website footers or social links, etc. Ignore them. if a url was used for synthesizing response, use markdown to properly format the reference link at the end of the response. For references, use a short display text instead of the website's entire title. Do not use the tool to fetch links that you already know the contents of. Do notify user if a particular link was unable to be scraped, ideally would return 'Couldnt scrape' as response"
        case .imageGeneration:
